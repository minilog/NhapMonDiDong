...
class MyListAdapter extends ArrayAdapter<String> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(resource == R.layout.list_item_1)
        {
            if (convertView == null) {
                LayoutInflater vi = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                convertView = vi.inflate(R.layout.list_item_1, null);
        ...
        if(resource == R.layout.list_item_2)
        {
            if (convertView == null) {
                LayoutInflater vi = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                convertView = vi.inflate(R.layout.list_item_2, null);
                viewHolder = new ViewHolder();
                viewHolder.txtWord = (TextView) convertView.findViewById(R.id.tvWord);
                viewHolder.btnSpeak = (Button) convertView.findViewById(R.id.btnSpeak);
                viewHolder.layout = (LinearLayout) convertView.findViewById(R.id.itemLayout);
            ...
            viewHolder.layout.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(getContext(), DongTuBatQuyTacDetailActivity.class);
                    intent.putExtra("word", listWord.get(position).toString());
                    getContext().startActivity(intent);
                }
            });
